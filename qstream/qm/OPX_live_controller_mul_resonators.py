...
class QuAM(QuamRoot):
    ...
    resonators: Dict[str, InOutSingleChannel_M] = field(default_factory = dict)
...
def make_quam_mul_resonantors(
    ...
    for i in range(len(resonator_elements.keys())):
        machine.resonators[list(resonator_elements.keys())[i]] = InOutSingleChannel_M(
            id=list(resonator_elements.keys())[i],
            opx_output=(controller, resonator_output),
            opx_input=(controller, resonator_input),
            intermediate_frequency=list(resonator_elements.values())[i],
...
class VirtualGateSetMeasurement:
    def __init__(
        ...
        for resonator_name in range(len(list.self.quam.machine.resonators)):
            self.quam.resonators[resonator_name].operations["readout"] = pulses.ConstantReadoutPulse(
...
class VirtualGateSetMeasurement_mul_resonantors:
    def __init__(
        self,
        qmm: QuantumMachinesManager,
        QuAM: QuAM,
...
class VirtualGateSetMeasurementBetter(VirtualGateSetMeasurement):
    def do_one_map(self, I_stream, Q_stream):
        ...
        with for_(
            *from_array(amplitude_scale_slow, np.linspace(-1, 1, self.resolution)[:self.resolution//2])
            ...
            # move ramp to zero here maybe, remove from do_one_row
            wait(100)
        
        assign(amplitude_scale_slow, 0)
        self.do_one_row(1, amplitude_scale_slow, I_stream, Q_stream)
        
        self.do_one_row(-1, amplitude_scale_slow, declare_stream(), declare_stream())
        wait(1000)


    def do_one_row(self, direction, amplitude_scale, I_stream, Q_stream,):
        small_jumps = declare(int)
        self.quam.align_all()
        self.quam.VirtualGateSet1.play("slow_pulse", direction * amplitude_scale)
    ...
    def reorder_rows(self, arr):
        rows_even = arr[::2]
        rows_odd = arr[1::2, ::-1] # last part might be wrong
