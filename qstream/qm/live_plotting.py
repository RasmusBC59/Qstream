...
from qm.qua import (wait_for_trigger,reset_phase,program,update_frequency, Math, Cast,
                   for_,stream_processing,declare,declare_stream,wait,measure,
                   play,save,fixed,demod,declare_input_stream,advance_input_stream,ramp,amp,if_,elif_,else_,align, ramp_to_zero, while_, pause, assign, IO1, infinite_loop_)
import numpy as np
...
class Live_plotting():
    def __init__(self,
        ...
        self.qm = qm
        readout_pulse_length = qm._config['pulses'][readout_pulse]['length']//4 #length in clockcycles
        self.readout_pulse = readout_pulse
        CW_amp = qm._config['waveforms']['const_wf']['sample']
        self.measured_element = measured_element

        self.program = self.make_program(gate1, gate2, gate1_range, gate2_range, gate1_points, gate2_points, readout_pulse, dividers, CW_amp, readout_pulse_length)

    def make_program(self, gate1, gate2, gate1_range, gate2_range, gate1_points, gate2_points, readout_pulse, dividers, CW_amp, readout_pulse_length):
        gate1_step = gate1_range*dividers[gate1]/(gate1_points-1)/CW_amp
        gate1_start = -gate1_range/2*dividers[gate1]/CW_amp #- gate1_step
        
        gate2_step = gate2_range*dividers[gate2]/(gate2_points-1)/CW_amp
        gate2_start = -gate2_range/2*dividers[gate2]/CW_amp #- gate1_step

        with program() as running_program:
            I = declare(fixed)
            Q = declare(fixed)  
            I_stream = declare_stream()
            Q_stream = declare_stream()
            gate1_step_index = declare(int)
            gate2_step_index = declare(int)
            
            with infinite_loop_():
                play('CW'*amp(gate1_start), gate1, 4)
                with for_(gate1_step_index, 0, gate1_step_index < gate1_points, gate1_step_index + 1):
                    play('CW'*amp(gate2_start), gate2, 4)

                    with if_(gate1_step_index==0):
                        wait(readout_pulse_length, gate1)
                    with else_():
                        play('CW'*amp(gate1_step), gate1, readout_pulse_length)

                    with for_(gate2_step_index, 0, gate2_step_index < gate2_points, gate2_step_index + 1):

                        with if_(gate2_step_index==0):
                            wait(readout_pulse_length, gate2)
                        with else_():
                            play('CW'*amp(gate2_step), gate2, readout_pulse_length)

                        measure(readout_pulse, 'bottom_left_DQD_readout', None, demod.full('cos', I), demod.full('sin', Q))
                        save(I,I_stream)
                        save(Q,Q_stream)
                    ramp_to_zero(gate2)
                ramp_to_zero(gate1)
            ...
            with stream_processing():
                I_stream.buffer(gate2_points,gate1_points).save_all('I')
                Q_stream.buffer(gate2_points,gate1_points).save_all('Q')
    ...
    def start_measurement(self, ):
        ...
        Warning('REMEMBER TO END MEASUREMENT')

    def fetch_results(self, return_mode = 'all'):
        ...
        if return_mode=='all':
            return i, q
        elif return_mode=='latest':
            return i[-1], q[-1]
        else:
            raise Exception(f'return mode: {return_mode} not recognized, accepted modes are: "all", "latest"')
        
    def end_measurement(self, ):
        print(self.running_job.halt())



class Live_plotting_spiral():
    """
    ...
    def __init__(self,
        ...
        self.program = self.make_spiral_program(resolution, gate1_range*dividers[gate1], gate2_range*dividers[gate2])
        self.order = spiral_order(resolution)

    def measurement_macro(self, I, Q, I_stream, Q_stream):
        measure(self.readout_pulse, self.measured_element, None, demod.full('cos', I), demod.full('sin', Q))
        save(I,I_stream)
        save(Q,Q_stream)

    def make_spiral_program(self, resolution, x_amp, y_amp):
            ...
            with infinite_loop_():
                ...
                with while_(completed_moves < resolution * (resolution - 1)):
                    ...
                    with for_(j, 0, j < moves_per_edge, j + 1):
                        ...
                        # playing the constant pulse to move to the next pixel
                        with else_():
                            play("CW" * amp(movement_direction * y_step_size), self.gate2)
    ...
    def fetch_results(self, return_mode = 'average'):
        if hasattr(self, 'data_fetcher'):
            I, Q= self.data_fetcher.fetch_all()

            amplitude = I**2 + Q**2 #Q[:I.shape[0],:,:]**2 # is this an issue ? will I and Q be offset in future fetches?
        else:
            raise Exception('data fetcher not started, run "start_measurement" before trying to fetch results')
...
class Live_plotting_spiral_modifiable():
    ...
    def make_spiral_program_modifiable(self, resolution): # resolution, x_amp, y_amp):
        ...
        with program() as spiral_scan:
            ...
            with stream_processing():
                for stream_name, stream in zip(["I", "Q"], [I_stream, Q_stream]):
                    stream.buffer(resolution**2).save(stream_name) #.buffer(resolution**2)
                x_st.save_all("x")
                y_st.save_all("y")
    ...
    def send_input_streams(self,):
        self.running_job.insert_input_stream('ranges_input_stream', [self.gate1_range, self.gate2_range])
